...
revision으로 두 번째 mutation을 실행하지 못하게 한다. project shell
재조회가 성공하면 함께 시작한 여섯 detail resource의 완료를 기다리지 않고
header busy와 transition 잠금을 해제한다. project header refresh 중에는
`프로젝트 정보 새로고침 중…` live 상태 문구를 제공한다.
