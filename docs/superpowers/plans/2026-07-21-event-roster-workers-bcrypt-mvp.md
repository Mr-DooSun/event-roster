...
- D1에는 `algorithm='bcrypt'`와 bcrypt hash만 저장한다. 원문 비밀번호·salt 원문·JWT 원문·CSRF 원문·복구 코드 원문·bcrypt hash·Worker secrets는 로그, 감사 로그, 일반 DTO, 브라우저 영구 저장소에 남기지 않는다. 단, CSRF 원문은 로그인 성공 `LoginSuccess.csrfToken` 및 인증된 `POST /auth/csrf`의 `no-store` 응답에서 provider 메모리로만 전달할 수 있고, bootstrap/사용자 발급·재설정/복구의 단회 임시 비밀번호 또는 복구 코드는 `no-store` 응답과 화면 메모리에서 한 번만 전달할 수 있다. 이 예외값도 Query cache·로그·감사에는 남기지 않는다.
...
- Workers Free의 요청당 50 D1 query와 statement당 100 bound-parameter 제약을 import planner가 지킨다. validate/commit은 행별 D1 호출을 금지하고 parameter-bound `VALUES` bulk statement와 UPSERT를 chunked로 사용한다. worst-case 130행의 읽기·guard·participant/roster/audit write·import run·revision·cleanup을 합쳐 50 query 미만, 각 statement 100 bindings 이하임을 unit test로 검증한다. JSON1에 의존하지 않는다.
...
모든 Task는 아래 이름을 그대로 사용한다. bcrypt hash, JWT 원문, recovery HMAC은 API DTO에 넣지 않는다. raw CSRF는 `LoginSuccess.csrfToken` 또는 인증된 `POST /auth/csrf`의 no-store 응답이라는 의도적인 일회성 예외에서만 DTO에 넣는다.
...
export interface LoginSuccess {
  ...
  /** no-store 응답에서만 수신하며 provider 메모리 외 저장/캐시를 금지한다. */
  csrfToken: string;
}
...
| `POST /api/v1/auth/login` | `{ loginId, password }` | `LoginSuccess`, 세션 쿠키, `Cache-Control: no-store` |
| `GET /api/v1/auth/me` | 없음 | `CurrentSession` |
| `POST /api/v1/auth/csrf` | 인증 쿠키, 정확한 `Origin` | 회전된 `{ csrfToken }`, `Cache-Control: no-store` |
...
it("returns 404 for a wrong probe token without running bcrypt", async () => {
  ...
  const app = createProbeApp(
    { DUMMY_BCRYPT_HASH: validDummyHash, CAPABILITY_PROBE_TOKEN: "probe-token" },
    { password: passwordSpy },
  );
  const response = await app.request("https://probe.test/probe?run=00000000-0000-4000-8000-000000000001", {
    method: "POST",
...
it("fails closed without bcrypt comparison when the configured dummy hash is invalid", async () => {
  const passwordSpy = {
    hash: vi.fn(),
    verify: vi.fn(),
    assertCostTwelveHash: vi.fn(() => { throw new Error("invalid_bcrypt_policy_hash"); }),
  };
  const app = createProbeApp(
  ...
  const response = await app.request("https://probe.test/probe?run=00000000-0000-4000-8000-000000000001", {
    method: "POST",
    headers: { "content-type": "application/json", "X-ER-Probe-Token": "probe-token" },
    body: JSON.stringify({ operation: "dummy" }),
  });
...
The intentionally nonexistent module in the RED state makes these tests fail to import first. Test-only hashes are generated at runtime and never committed. `createProbeApp(env, { password })` is the test seam; the default Worker export creates the app with the real bcrypt hasher. Invalid dummy-hash configuration must return a generic 500 with no hash/token/error detail.
...
Set `.nvmrc` to `22`. Add `hono`, `zod`, `jose`, `@cloudflare/vitest-pool-workers`, `wrangler`, React, React Router, TanStack Query, React Hook Form, Vite, `vitest@^4.1.0`, and TypeScript to the correct workspace package only. The web package must explicitly include `@testing-library/react`, `@testing-library/user-event`, `@testing-library/jest-dom`, and `jsdom` as test dependencies; Worker TypeScript is strict with Bundler resolution, WebWorker lib, and `esModuleInterop: true`; Task 6 adds `bcryptjs` to the Worker package. Add `apps/worker/.wrangler`, `apps/worker/.dev.vars`, E2E secrets/state, Python caches, and Playwright artifacts to `.gitignore`.

Configure React tests before running them:
...
it("uses only cost-12 hashes and rejects a dummy hash at another cost", async () => {
  ...
  const costTenHash = await bcrypt.hash("test-only-cost-ten-password", 10);
  expect(() => hasher.assertPolicyHash(costTenHash)).toThrow();
});
...
Use `jose` HS256 and reject altered signature, issuer, audience, malformed claims, expiry, and wrong `kind`. FULL JWT and cookie expire in 8 hours; MUST_CHANGE_PASSWORD expires in 10 minutes. `problem.ts` never returns caught exception text. `request-context.ts` accepts only `CF-Connecting-IP`, HMACs it with `IP_HASH_KEY` before any storage, and uses one explicit documented fallback bucket when the header is absent; it never reads `X-Forwarded-For`. `origin.ts` rejects every authenticated state-changing route—including `POST /auth/csrf`, because it rotates state—with Origin not exactly `APP_ORIGIN`.

`csrf.ts` creates 32 random bytes encoded as base64url, stores only `SHA-256(rawToken)` in the current `auth_sessions.csrf_hash`, and compares the SHA-256 of `X-ER-CSRF` against that session's stored digest with XOR constant-time bytes. `POST /auth/csrf` requires an authenticated session plus exact Origin, atomically rotates the stored digest without requiring the old CSRF header, then returns the new raw token with `Cache-Control: no-store`; raw CSRF is never put in JWT, logs, browser storage, or query cache. Add tests for session A token rejected by session B, rotation invalidating prior token, missing/mismatched Origin on the POST route, and no raw token in D1 rows.
...
it("plans a worst-case 130-row commit within Workers Free D1 budgets", () => {
  const plan = buildImportQueryPlan(makeResolvedRows(130), { everyRowIsNew: true });
  expect(plan.totalQueries).toBeLessThan(50);
  expect(plan.statements.every((statement) => statement.bindingCount <= 100)).toBe(true);
});
```
...
Workers Free D1 budget is a correctness requirement, not a performance wish. `buildImportQueryPlan()` must reject impossible input and return the exact query/binding budget before any D1 call. `validate` and `commit` may perform only bounded set reads; they must never loop over rows with `.run()`, `.first()`, or `.all()`. Commit uses exactly three pre-batch reads (event/scope, participant+organization candidates, current event roster) and one guarded `db.batch()`. The batch uses parameter-bound `WITH constants AS (...)` plus chunked `VALUES` input and `INSERT ... ON CONFLICT` so participant creation, roster insertion/reactivation/no-op, and audit writes never require one statement per row or a separate reactivation UPDATE.

The planner carries immutable constants such as event ID, actor ID, action, and timestamp once per statement. Its variable tuples are capped at six parameters for participant rows, six for roster rows, and four for audit rows. At 130 all-new rows this yields at most 9 participant chunks (16 rows each), 9 roster chunks (16 rows each), and 6 audit chunks (24 rows each). With 3 set reads, the guard, import-run insert, event revision update, and guard cleanup, the worst-case total is `3 + 1 + 9 + 9 + 6 + 3 = 31` D1 queries; each statement has at most 100 bound parameters. The planner must preserve this `<= 31` budget in its exported metadata and reject a generated plan above it. It must not depend on JSON1, interpolate client values into SQL, or silently fall back to row-by-row D1 calls.

- [ ] **Step 4: Verify and commit**

Add unknown/inactive organization, duplicate file row, ambiguity requiring a resolution, accepted candidate resolution, stale/foreign candidate rejection, role/DAY_OF rejection, reactivation/no-op, stale expected revision zero-write, exact export ordering, and worst-case 130-row `<= 31` query/`<= 100` binding planner-budget tests. Run suite/check; expected PASS. Commit `feat: add atomic roster import and export data`.
...
**Interfaces:**
- Consumes: Task 7 auth endpoints, `CurrentSession`, and `LoginSuccess`.
...
Define `--er-color-*`, `--er-space-*`, `--er-radius-*`, `--er-shadow-*`, `--er-font-weight-*`; all UI primitives consume these tokens. `api.ts` uses relative `/api/v1`, `credentials: "include"`, JSON content, and adds `X-ER-CSRF` only from provider memory. It never stores password, bcrypt hash, JWT, recovery code, temporary password, or CSRF token in persistent storage/query cache.

`AuthProvider` first calls `GET /auth/me`; when it finds a session, it immediately calls credentialed `POST /auth/csrf` and keeps the returned raw token only in provider state. A successful login consumes `LoginSuccess`, places `session` and `csrfToken` directly in provider state, and does not make an unnecessary second rotation. Every mutation reads only that memory value. Logout, successful password change, `401`, and `INVALID_CSRF` clear it. The `POST /auth/csrf` bootstrap request relies on the browser-sent exact Origin and intentionally sends no prior CSRF header.

`LoginPage` has `로그인 ID`, generic invalid-credential copy, and recovery link. Login, change-password, recovery, bootstrap handoff, and user reset forms all use the shared 72-byte UTF-8 password schema and show its Korean validation message before submit. `ChangePasswordPage` is the only MUST_CHANGE route, clears provider memory after 204, and shows `새 비밀번호로 다시 로그인하세요.` `RecoveryPage` and `BootstrapHandoffPage` keep one-time values only in dialog state and clear them on close/unmount. Guards route no session to `/login`, MUST_CHANGE to `/change-password`, bootstrap FULL to `/bootstrap-handoff`, and ordinary FULL to `/events`. Add UI tests that the login response token enables an immediate MUST_CHANGE password change and bootstrap handoff, a refresh reacquires the token through `GET /auth/me` then `POST /auth/csrf`, and no raw token reaches browser storage or a React Query cache.
...
- [x] Login `LoginSuccess` and authenticated `POST /auth/csrf` give the in-memory provider a no-store CSRF bootstrap/rotation path without weakening exact-Origin validation.
...
- [x] A 130-row worst-case import uses an explicit `<= 31` D1 query/`<= 100` bindings-per-statement bulk planner test compatible with Workers Free; React tests explicitly use jsdom and Testing Library setup.
