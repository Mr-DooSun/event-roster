...
2. 프로젝트 종료 뒤 별도 활성 조직을 새로 연결한다. 또 다른 조직을 사용
   중지하거나 삭제한 뒤 `이력 보정 시작`으로 진입해 연결한다. 학생·교사 추가,
   당시 snapshot 수정, 한 행 취소·복원, `mode=history-correction` Excel
   검증·확정을 수행한다.
...
5. 종료 전에 연결돼 있던 활성 조직의 담당자로 로그인해 같은 종료 프로젝트가
   읽기 전용인지 확인한다. `이력 보정 시작`이 없어야 하며 correction 후보
   read와 write 요청은 `403`이어야 한다.
6. 2번에서 종료 후 처음 연결한 활성 조직의 담당자로 로그인한다. 프로젝트가
   목록에 없어야 하고, 프로젝트 상세·조직·명단·집계·감사·내보내기 직접 요청은
   모두 `403`이어야 한다. 이어 2번의 비활성·삭제 조직을 재활성화·복구하고 그
   담당자로 다시 로그인해도 같은 목록 제외와 `403`을 확인한다. 하나라도
   manager scope에 들어오면 배포 실패로 기록한다.
7. 두 운영자 탭에서 같은 revision을 읽고 모두 보정 mode에 진입한다. 첫 탭에서
