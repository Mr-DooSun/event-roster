...
it("exports deterministic roster and complete summary arrays without auth fields", async () => {
  ...
  await env.DB.prepare(
    `INSERT INTO event_roster_entries
     ...
     VALUES ('same-entry', ?, 'same-name', 'org-1', '첫 참가자', '1팀',
             'PRE_EVENT', 'ACTIVE', 0, 0, 'user-1', 'user-1', ?, ?)`,
  )
    .bind(
      fixture.event.id,
  ...
  const response = await authedRequest(
    fixture.operator,
    `/api/v1/events/${fixture.event.id}/export-data`,
  );
  const body = await response.json<{
    명단: Array<{ "고유 ID": string; 이름: string; "최종 수정": string }>;
    집계: Array<{ "당일 추가": number; "당일 취소": number }>;
  }>();
  ...
  expect(body.집계[0]).toMatchObject({ "당일 추가": 0, "당일 취소": 0 });
